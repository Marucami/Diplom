...
from api.models import Account, Transaction, Category, RecurringTransaction
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            login(request, user)
            return Response(
                {"id": user.id, "username": user.username, "authenticated": True}
            )
...
class TransactionView(APIView):
    ...
    def get(self, request):
        queryset = Transaction.objects.filter(owner=request.user).order_by(
            "-date", "-id"
        )
    ...
    def get(self, request):
        categories = Category.objects.filter(owner=request.user).annotate(
            total_balance=Coalesce(
                Sum("transactions__amount"), Value(0.0), output_field=DecimalField()
            )
        )
...
class AnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        monthly_stats = (
            ...
            )  # <-- ИСПРАВЛЕНО ЗДЕСЬ (просто TruncMonth)
            .values("month", "type")
        ...
        current_month_txs = Transaction.objects.filter(
            owner=user, date__gte=current_month_start, date__lte=now
        ).values("type", "amount")
...
class RecurringTransactionView(APIView):
    ...
    @extend_schema(responses=RecurringTransactionSerializer(many=True))
    def get(self, request, pk=None):

        if pk:
            recurring = get_object_or_404(
                RecurringTransaction, pk=pk, owner=request.user
            )

            serializer = RecurringTransactionSerializer(recurring)
            return Response(serializer.data)

        recurring = RecurringTransaction.objects.filter(owner=request.user).order_by(
            "-created_at"
        )

        serializer = RecurringTransactionSerializer(recurring, many=True)

        return Response(serializer.data)

    @extend_schema(
        request=RecurringTransactionSerializer,
        responses=RecurringTransactionSerializer,
    )
    def post(self, request):
    ...
    def put(self, request, pk):
        ...
        recurring = get_object_or_404(RecurringTransaction, pk=pk, owner=request.user)

        serializer = RecurringTransactionSerializer(
            recurring, data=request.data, partial=True, context={"request": request}
        )
